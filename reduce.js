...
let maxCallback = ( acc, cur ) => {return { x: Math.max( acc.x, cur.x )}};
...
reduce	Full support 3	Full support 12	Full support 3	Full support 9	Full support 10.5	Full support 5	Full support ≤37	Full support 18	Full support 4	Full support 14	Full support 4	Full support 1.0	Full support 0.1.100
What are we missing?
Legend
...
Metadata
    ...
    Last modified: Oct 12, 2020, by MDN contributors
...
© 2005-2020 Mozilla and individual contributors. Content is available under these licenses.
    ...
    Cookies
`;
console.log('Webpage in the day of codewriting:')
console.log(content);

const utils = {
    isCharValid: char => char.match(/[a-z0-9]/i),
    turnToLowercase: char => char.toLowerCase(),
    instanceCounter: function(counts, char) {
        counts[char]
        ? counts[char] = counts[char] + 1
        : counts[char] = 1;
        return counts; // so the next loop can use it;
    },
    sortByValue: (a, b) =>  a[1] - b[1],
};
console.log('// cleaning the junk:')
console.log('if a character is a-z || A-Z || 0-9 then keep it; convert it to lowercase; count each char;')
// cleaning the junk:
// if a character is a-z || A-Z || 0-9 then keep it;
const contentCleaned = content
    .split('') // split each char into an item of an array;
    .filter(utils.isCharValid)
    .map(utils.turnToLowercase) // convert everything to lowercase;
    .reduce(utils.instanceCounter, {});

console.log(contentCleaned);

// sort it by the number of occurance
console.log('// sort it by the number of occurance')
const resultSorted = Object
    .entries(contentCleaned)
    .sort(utils.sortByValue);
